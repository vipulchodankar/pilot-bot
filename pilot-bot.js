...
client.on("ready", () => {
  ...
  const attachment = new Discord.Attachment(
    "https://vetstreet.brightspotcdn.com/dims4/default/60b6ebe/2147483647/thumbnail/590x420/quality/90/?url=https%3A%2F%2Fvetstreet-brightspot.s3.amazonaws.com%2F1b%2F10%2F187d796e44db9b3262b45d2e5aac%2Fdog-makes-eye-contact-thinkstockphotos-511375254-590lc031616.jpg"
  );
...
function processCommand(recievedMessage) {
  ...
  switch (primaryCommand) {
    ...
    case "hey":
    case "hi":
      hiCommand(recievedMessage);
      break;

    case "ping":
      pingCommand(recievedMessage);
      break;
    ...
    case "add":
      addCommand(arguments, recievedMessage);
      break;
    ...
    case "picture":
    case "pic":
      getPic(arguments, recievedMessage);
      break;

    default:
      recievedMessage.channel.send(
        "Sorry, i didn't understand what you said. Try again?"
      );
      break;
  }
}
...
function hiCommand(recievedMessage) {
  ...
  recievedMessage.channel.send(
    greeting[Math.floor(Math.random() * 5)] +
      " " +
      recievedMessage.author.toString()
  );
}
...
function addCommand(arguments, recievedMessage) {
  ...
  recievedMessage.channel.send(
    "The sum of " + arguments + " is " + sum.toString()
  );
}
...
const getImage = async parameter => {
  try {
    return await axios.get(
      "http://www.splashbase.co/api/v1/images/" + parameter
    );
  } catch (error) {
    console.error(error);
  }
